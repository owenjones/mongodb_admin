...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-o",
        action="store_true",
        help="save csv output for successful and unsuccessful modifications",
    )
    parser.add_argument(
        "--debug",
        "-v",
    ...
    debug(f"Run finished: { len(successful) } user(s) modified.")

    if len(unsuccessful) > 0 and args.debug:
        debug(
            f"\nThere was a problem modifying { len(unsuccessful) } user(s):"
        )
